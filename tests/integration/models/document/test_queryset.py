...
class TestDocumentQuerysetBulkOperations(UnitTestCase):
    ...
    def _cleanup_test_documents(self) -> None:
        ...
        try:
            ...
            if test_docs:
                ...
                if doc_ids:
                    self.client.documents.bulk_delete(doc_ids)
    ...
    def test_bulk_modify_tags(self) -> None:
        ...
        queryset.bulk_modify_tags(add_tags=[self.tag_id])
    ...
    def test_bulk_add_tag(self) -> None:
        ...
        # Apply the bulk operation using queryset
        queryset = self.client.documents().filter(id__in=doc_ids)
        queryset.bulk_add_tag(self.tag_id)
    ...
    def test_bulk_remove_tag(self) -> None:
        ...
        # Apply the bulk operation using queryset
        queryset = self.client.documents().filter(id__in=doc_ids)
        queryset.bulk_remove_tag(self.tag_id)
    ...
    def test_bulk_set_correspondent(self) -> None:
        """Test bulk_set_correspondent operation."""
        ...
        # Apply the bulk operation using queryset
        queryset = self.client.documents().filter(id__in=doc_ids)
        queryset.bulk_set_correspondent(self.correspondent_id)
    ...
    def test_bulk_set_document_type(self) -> None:
        ...
        # Apply the bulk operation using queryset
        queryset = self.client.documents().filter(id__in=doc_ids)
        queryset.bulk_set_document_type(self.document_type_id)
    ...
    def test_bulk_set_storage_path(self) -> None:
        ...
        # Apply the bulk operation using queryset
        queryset = self.client.documents().filter(id__in=doc_ids)
        queryset.bulk_set_storage_path(self.storage_path_id)
    ...
    def test_bulk_merge(self) -> None:
        """
        Test bulk_merge operation.
    ...
    def test_bulk_reprocess(self) -> None:
        """
